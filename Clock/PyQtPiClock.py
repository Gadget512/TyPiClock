...
class Calendar():
	...
	def getEvents(self, day=None, month=None, year=None):
		...
		# TODO 
		offset = '-07:00' # MST (winter), -06:00 MDT (summer)
...
def testCalendar():
	...
	#Tomorrow
	tom = datetime.datetime.utcnow()
	tom = tom.replace(day=tom.day+1) # TODO day+1 doesn't work across months
	
	"""
	eventsTod = calendar.getEvents(date=now)
	...
	eventsTom = calendar.getEvents(date=tom)
	...
	eventsMonth = calendar.getEventsMonth(date=now)
	
	if not eventsMonth:
		print('No events this month')
	else:
		print('This month\'s events:')
